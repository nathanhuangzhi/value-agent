...
- **Foreign filers (ADRs):** SEC facts are read in the filer's reporting currency (`companies_sec.json[].currency`), 20-F/6-K forms included; `app/tools/fx.py` converts blended statements to USD per cell source at the two blend sites (API + report), using `data/fx_rates.json` (daily, yfinance `CNY=X`…). ADS ratio is inferred from SEC-vs-yfinance share counts and SEC shares/EPS are put on the ADS basis (`sec_adapter.infer_ads_ratio`). Quarterly results for these companies come from 6-K press releases extracted by DeepSeek (`scripts/fetch_6k_statements.py` → `data/sec_6k/<T>.json`, every line item + standard metrics) and blended as a third source: SEC XBRL > 6-K > yfinance (`sec_adapter.overlay_source_row`).
