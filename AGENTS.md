...
- **Raw upstream caches (box only, gitignored):** `data/sec_raw/CIK*.json.gz` (SEC companyfacts) and `data/yfinance_raw/<T>.json` (the six statement frames). `companies_sec.json` / `data/yfinance/` are *views* over them — after adding an XBRL concept or a yfinance label, run `fetch_sec_annual --reparse` / `fetch_yfinance_statements --reparse` (seconds, no network) instead of re-downloading.
