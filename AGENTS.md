...
./venv/bin/python -m scripts.fetch_sec_annual [--ticker QDEL]        # Stage 4b: SEC XBRL → data/sec/<T>.json (free, ~30s)
...
- `app/db.py` + `app/auth/` — user data. One SQLite (`data/app.db`, box only): users, sessions, login codes, per-user watchlists, custom metrics/charts. Sign-in is email + 6-digit code (`/auth/code`, `/auth/verify`, `/me`), sessions are bearer tokens; `app.auth.deps.current_user` guards `/ai/*` and `/watchlist`. Conversations carry `user_id` in their JSON. `data/watchlist.json` stays the pipeline's input — it is rewritten as the union of every user's list on each change. First-time setup: `scripts/migrate_accounts.py --email …` hands the pre-account data to the first user.
...
`.gitignore` ignores `data/*` then whitelists the pipeline **state files**, so those are tracked (a fresh clone / the cron box starts with the candidate universe already on disk). Tracked: `companies.jsonl`, `companies_classified.json`, `companies_filtered.json`, `companies_analyzed.json`, `companies_validation.json`, `companies_digest.json`, `daily_industry_log.json`, `classify_probes.json`, `companies_classify_failures.json`, and `yfinance/*.json`. **Not** tracked: `data/sec/<TICKER>.json` (the SEC XBRL rows, one file per ticker — re-fetchable) and `reports/*.html`.

Flow: `companies.jsonl` (universe) → `companies_classified.json` → `companies_filtered.json` → `companies_analyzed.json` (narratives + price history). Financial statements live in **sidecars**, not in the analyzed row: `data/sec/<T>.json` (primary, SEC XBRL; read through `app.tools.sec_store.SecStore`, never parsed whole by the service) and the yfinance gap-fill (same row shape, per-industry shards). `companies_validation.json` holds per-ticker issue lists. The renderer reads all three and surfaces a banner when validation status ≥ warn. `reports/` holds rendered HTML (per-ticker + `index.html` + `<industry-slug>.html`; `_digest.html` is the email body).
...
- **DeepSeek is OFF by default** (`LLM_DEFAULT=off` in `deploy/run_daily.sh`, decided 2026-09-13). Off = `classify_companies` skipped, `daily_scan --no-llm` (batches still rotate and price history refreshes, but no Exa/DeepSeek narrative — an existing narrative on the row is kept, never wiped), and `daily_digest --skip-summary` (email goes out without a "Stories of the day" block; stored summaries are left untouched). Set `LLM_DEFAULT=on` (or `LLM=on` for one run) to resume narratives + summaries. Narratives/summaries are hidden by matching switches: `mobile/src/config.ts: SHOW_LLM_ANALYSIS` (app) and `app/tools/report/render.py: SHOW_LLM_ANALYSIS` (web report); the email omits its block whenever `summary_md` is empty.
- **Foreign filers (ADRs):** SEC facts are read in the filer's reporting currency (`data/sec/<T>.json` → `currency`), 20-F/6-K forms included; `app/tools/fx.py` converts blended statements to USD per cell source at the two blend sites (API + report), using `data/fx_rates.json` (daily, yfinance `CNY=X`…). ADS ratio is inferred from SEC-vs-yfinance share counts and SEC shares/EPS are put on the ADS basis (`sec_adapter.infer_ads_ratio`). Quarterly results for these companies come from 6-K press releases extracted by DeepSeek (`scripts/fetch_6k_statements.py` → `data/sec_6k/<T>.json`, every line item) then mapped onto Yahoo-aligned standard definitions by `scripts/remap_6k_standard.py` (`app/prompts/remap_6k.prompt.md`; bump `STANDARD_VERSION` after a definition change to re-map everything, ~$0.5) and blended as a third source: SEC XBRL > 6-K > yfinance (`sec_adapter.overlay_source_row`).
- **Raw upstream caches (box only, gitignored):** `data/sec_raw/CIK*.json.gz` (SEC companyfacts), `data/yfinance_raw/<T>.json` (the six statement frames) and `data/sec_annual_reports/<T>/` (the last three annual reports — 20-F / 10-K — as filed, plus flattened text and a per-filing table of contents in `<T>.json`; `scripts/fetch_annual_reports.py`, watchlist by default, `--reparse` after a text/TOC change). The annual reports feed only the AI chat's `list_annual_reports` / `get_annual_report_section` / `search_annual_report` tools (`app/ai/context.py`), which also fetch a company's latest report on demand. Likewise `data/earnings_calls/<T>.json` holds earnings-call transcripts from Alpha Vantage (`scripts/fetch_earnings_calls.py`, watchlist, ≤15 requests per key per run; `app/tools/earnings_calls.py`) for the chat's `list_earnings_calls` / `get_earnings_call` / `search_earnings_calls` tools. `data/sec/` / `data/yfinance/` are *views* over them — after adding an XBRL concept or a yfinance label, run `fetch_sec_annual --reparse` / `fetch_yfinance_statements --reparse` (seconds, no network) instead of re-downloading.
- **SEC cache freshness:** the daily run passes `--max-age-days 7` to re-download a rolling ~1/7 of the pool each day (refreshing the raw file too).
- **Watchlist:** the app's Saved tab syncs to the signed-in user's list (`/watchlist`, app.db); `data/watchlist.json` is the union across users and is what the pipeline reads; `daily_scan` adds those tickers to the day's batch once per cycle even when they're outside the filtered pool. `daily_scan --watchlist-only` runs just them.
- **Steps (LLM=on):** classify → filter → daily_scan → `daily_digest --publish-dir ~/public/value-agent` (SEC fetch + yfinance gap-fill + validate + reports + index + bake `/api` + publish + email), then commits the whitelisted state files back to this repo as `value-agent-bot` over an SSH deploy key (rebase-on-push retry loop).
...
- **The SEC rows live on the box** (`data/sec/`, ~400 MB across ~1,250 files, not committed), so `fetch_sec_annual` is incremental; only a fresh box needs a one-off warm-up. The old `companies_sec.json` monolith was split by `scripts/migrate_sec_shards.py` on 2026-09-17.
...
> A dev checkout still has only a partial SEC cache — never bake + publish
> `/api` from a dev box.
