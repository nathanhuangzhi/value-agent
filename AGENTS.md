...
- **DeepSeek is OFF by default** (`LLM_DEFAULT=off` in `deploy/run_daily.sh`, decided 2026-09-13 because the candidate pool was exhausted). Off = classify / daily_scan skipped and `daily_digest --skip-summary` (reuses the stored summary). Set `LLM_DEFAULT=on` (or `LLM=on` for one run) to resume analysis.
