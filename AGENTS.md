...
- **DeepSeek is OFF by default** (`LLM_DEFAULT=off` in `deploy/run_daily.sh`, decided 2026-09-13). Off = `classify_companies` skipped, `daily_scan --no-llm` (batches still rotate and price history refreshes, but no Exa/DeepSeek narrative — an existing narrative on the row is kept, never wiped), and `daily_digest --skip-summary` (email goes out without a "Stories of the day" block; stored summaries are left untouched). Set `LLM_DEFAULT=on` (or `LLM=on` for one run) to resume narratives + summaries. The mobile app hides narratives/summaries behind `mobile/src/config.ts: SHOW_LLM_ANALYSIS`.
