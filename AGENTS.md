...
- **Watchlist:** the app's Saved tab syncs to `data/watchlist.json` via `/watchlist` (served by the AI uvicorn); `daily_scan` adds those tickers to the day's batch once per cycle even when they're outside the filtered pool. `daily_scan --watchlist-only` runs just them.
